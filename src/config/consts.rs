...
mod _unix_dirs {
    ...
    pub const SYSTEM_RUNTIME_DIR: &str = concat!("/run/", env!("CARGO_BIN_NAME"));
    /// From `man 5 systemd.exec` (StateDirectory)
    pub const SYSTEM_STATE_DIR: &str = concat!("/var/lib/", env!("CARGO_BIN_NAME"));
    /// From `man 5 systemd.exec` (CacheDirectory)
    pub const SYSTEM_CACHE_DIR: &str = concat!("/var/cache/", env!("CARGO_BIN_NAME"));
    /// From `man 5 systemd.exec` (LogsDirectory)
    pub const SYSTEM_LOGS_DIR: &str = concat!("/var/log/", env!("CARGO_BIN_NAME"));
    /// From `man 5 systemd.exec` (ConfigurationDirectory)
    pub const SYSTEM_CONFIGURATION_DIR: &str = concat!("/etc/", env!("CARGO_BIN_NAME"));
}
#[cfg(target_family = "unix")]
pub use _unix_dirs::*;
